...
class Course_controller extends CI_Controller {
	...
	public function getAllCourses () {
		if ($this->session->userdata('profile') === null){
			...
			$subject_data = $this->db->select('*')->from('subjects')->join('teachers','subjects_idsubjects = idsubjects')->get();
			...
			if ($subject_data->num_rows() > 0) {
				...
				foreach ($subject_data->result_array() as $key => $value) {
					...
					$courses = $this->db->select('*')->from('courses')->where('subjects_idsubjects', $value['idsubjects'])->get();
					$ww = $value;
					$ww['courses'] = $courses->result_array();
					$dt[] = $ww;
				}
				...
				echo json_encode(array('error' => 'No courses found in the system'));
			}
